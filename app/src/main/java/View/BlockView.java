...
public class BlockView extends View{
    ...
    private void drawBlock(Canvas canvas) {
        ...
        for(int i=0; i<block.getSize(); ++i){//todo
            float startX = center + block.getPoint(i).x*cellSize - ((dimensions.x-block.getPoint(i).x)/2.0f*cellSize);
            float startY = center + block.getPoint(i).y*cellSize - ((dimensions.y-block.getPoint(i).y)/2.0f*cellSize);
